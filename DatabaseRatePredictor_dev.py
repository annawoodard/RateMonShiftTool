...
def main():
    try:
        ...
        pickYear()
        try:
            opt, args = getopt.getopt(sys.argv[1:],"",["makeFits","secondary","fitFile=","json=","TriggerList=","maxdt=","All","Mu","HCal","Tracker","ECal","EndCap","Beam","NoVersion","linear","inst","TMDerr","AllTriggers"])
...
###### TRIGGER LIST #######
        ...
        if trig_list == [] and all_triggers==False:
...
##                     else:
                ...
                if (first_trigger):
                        print '%-60s | line | %.2s| %.2s |   %.2s | %.1s |   %.2s | %.1s |   %.2s | %.1s |   %7.2s |   %4.0s |   %5.3s | ' % ('TRIGGER', 'X1', str('X1 ERROR'), 'X2', 'X2 ERROR', 'X3', 'X3 ERROR', '0','0','CHI^2', 'NUMBER DOF', 'CHI^2/NDOF')
                        first_trigger=False                
                print '%-60s | line | % .2f | +/-%.2f |   % .2e | +/-%.1e |   % .2e | +/-%.1e |   % .2e | +/-%.1e |   %7.2f |   %4.0f |   %5.3f | ' % (print_trigger, f1a.GetParameter(0) , f1a.GetParError(0) , f1a.GetParameter(1) , f1a.GetParError(1) , f1a.GetParameter(2), f1a.GetParError(2), 0                  , 0                 , f1a.GetChisquare() , f1a.GetNDF() , f1a.GetChisquare()/f1a.GetNDF())
